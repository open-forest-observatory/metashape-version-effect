...
eval_stats_dir = datadir("meta200/itd-eval-initialvwfsearch/tree_detection_evals")
...
d = d |>
  mutate(vwf_intercept = predicted_tree_dataset_name %>% str_split("_") %>% map(12)) |>
  mutate(vwf_slope = predicted_tree_dataset_name %>% str_split("_") %>% map(13))
